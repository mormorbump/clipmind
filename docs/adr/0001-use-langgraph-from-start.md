...
## ステータス
Accepted — 2026-04-25（2026-04-30 文言修正、2026-06-10 バージョン pin 改訂）
...
- LangGraph のバージョンは `langgraph >= 1.2, < 2` に pin して破壊的変更を避ける
  - Phase 1 着手時に PyPI 最新安定版（**1.2.4**）を確認して採用
  - Checkpointer は `langgraph-checkpoint-sqlite >= 3.1, < 4`
...
- **2026-06-10**: バージョン pin を `>=0.2,<0.3` から `>=1.2,<2` に更新（PyPI 最新安定版 1.2.4）。
  LangGraph は 2025 年中に 1.0 GA を迎え、API が安定化したため、初期方針の 0.2 系から最新メジャーへ切替。
  Checkpointer も `langgraph-checkpoint-sqlite >=3.1,<4` を採用。Phase 1 で依存追加と同時に確定。
