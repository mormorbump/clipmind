...
# サーバサイド (Streamlit プロセス) から API を呼ぶ際の接続先。
# k8s プレビュー環境ではクラスタ内 DNS (http://clipmind) が注入される。
DEFAULT_BASE_URL = os.getenv("CLIPMIND_API_URL", "http://localhost:8000")

# ブラウザに渡す URL (キーフレーム画像など <img src> で参照されるもの) の接続先。
# サーバサイドの DNS はブラウザから解決できないため、外部公開ホストを別系統で持つ。
# 未設定ならサーバサイドと同じ (ローカル開発では両者は一致する)。
PUBLIC_BASE_URL = os.getenv("CLIPMIND_PUBLIC_API_URL", DEFAULT_BASE_URL)
